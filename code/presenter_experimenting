...
def experimenting(tweets, award, regex_pairs):
    ...
    while True:
        ...
        cleaned = clean(
            potential_winners, ['RT', '@', 'Golden', 'Globe', 'Award', 'HBO'])
        ...
        if len(percent_dict.values()) < 2 or list(percent_dict.values())[0] / list(percent_dict.values())[1] >= 1.5:
            return(counts_dict, percent_dict, potential_winners, potential_winners, award_tweets, award_pos)
        # otherwise, split on for to try to further isolate the real winner
            ...
            cleaned = clean(
                potential_winners, ['RT', '@', 'Golden', 'Globe', 'Award'])
            cleaned2 = exclude_award_name(cleaned, award)
